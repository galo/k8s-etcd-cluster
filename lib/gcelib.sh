...
function create_gcloud_disk() {
	...
	case "x${DISKZONE}" in 
		...
		;;
		* )
			NBZONES=$(grep ${DISKZONE} "${MYDIR}/../etc/${CLOUD}-valid-zones" | wc -l)
			if [ "${NBZONES}" = "0" ] ; then
	...
	DISKTYPEZONE=$(grep ${DISKZONE} "${MYDIR}/../etc/${CLOUD}-valid-disk-options" | grep "${DISKTYPE}" | wc -l)
	...
	MINDISKSIZE=$(grep ${DISKZONE} "${MYDIR}/../etc/${CLOUD}-valid-disk-options" | grep "${DISKTYPE}" | cut -f3 -d";" | cut -f1 -d"-" | tr -d "GB")
	MAXDISKSIZE=$(grep ${DISKZONE} "${MYDIR}/../etc/${CLOUD}-valid-disk-options" | grep "${DISKTYPE}" | cut -f3 -d";" | cut -f2 -d"-" | tr -d "GB")
	...
	#DISKIMAGELIST=$(grep ${DISKIMAGE} "${MYDIR}/../etc/${CLOUD}-valid-images" | wc -l)
