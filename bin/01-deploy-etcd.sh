...
ETCDCONF="${MYDIR}/../etc/etcd.conf"
MYLIB="${MYDIR}/../lib/bashlib.sh"

for file in "${MYCONF}" "${MYLIB}" "${ETCDCONF}" "${MYDIR}/../lib/gcelib.sh" ; do
	[ -f ${file} ] && source ${file} || {
...
log debug Creating a discovery URL for etcd
...
while [ "x${ETCD_DISCOVERY_URL}" = "x" ]; do
	...
	log debug Found URL "${ETCD_DISCOVERY_URL}"
	sleep 1
done
...
log debug Using ${ETCD_CLUSTER_ID} as cluster name
...
while [ "$(kubectl describe service etcd-service | grep "LoadBalancer Ingress" | wc -l)" = "0" ]; do
	log info Service etcd-service not ready. Waiting.
	sleep 10;
done
...
log debug Sleeping a little bit to let containers spin
sleep 20
...
log debug Found etcd Service Ingress Load Balancer at ${CLUSTER_ETCD_SERVICE_SERVICE_HOST} on port ${CLUSTER_ETCD_SERVICE_SERVICE_PORT_CLIENT}
log debug Cluster version is $(curl -sL http://${CLUSTER_ETCD_SERVICE_SERVICE_HOST}:${CLUSTER_ETCD_SERVICE_SERVICE_PORT_CLIENT}/version)
